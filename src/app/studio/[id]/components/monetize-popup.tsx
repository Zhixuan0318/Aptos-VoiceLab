...
export default function MonetizePopup(paramGeral: any) {
    ...
    const { generateNewCard } = useAptos();
    ...
    async function sendNewVoicecardNFT() {
        try {
            ...
            paramGeral.setIsGenerated(true);
            paramGeral.setVoiceId(json.voice_id);
        } catch (error) {}
    }
    ...
    return (
        <div
            ...
            } inset-0 w-screen h-screen fixed flex flex-col transition duration-300 z-10`}
        >
            {!openGenerateCardConfirmation ? (
                ...
                >
                    <button
                        className='fixed w-screen h-screen blur opacity-50 bg-neutral-300'
                    ...
                    <div className='flex flex-col my-auto p-6 mx-auto items-center rounded-lg bg-white z-10 w-[950px]'>
                        <div
                            className={`${
                                fillData ? 'opacity-100' : 'opacity-0'
                            } transition-opacity duration-300 fixed flex w-full pointer-events-none`}
                        >
                            <p className='px-4 p-2 mx-auto rounded-lg bg-black text-red-600'>
                        ...
                        <div className='flex flex-row w-full'>
                            ...
                            >
                                <XMarkIcon className='w-6 text-neutral-400' />
                            </button>
                        </div>
                        <div className='flex flex-row w-full text-xs gap-16'>
                            <div className='flex flex-col w-full'>
                                <label htmlFor='' className='mb-2 mt-8'>
                                    Name
                                </label>
                                <input
                                    onChange={(e) => setName(e.target.value)}
                                    type='text'
                                    placeholder='Example: Daniel - Podcast'
                                    className='border rounded px-4 p-2'
                                    required
                                />
                                <label htmlFor='' className='mb-2 mt-6'>
                                    Description
                                </label>
                                <textarea
                                    onChange={(e) => setDescription(e.target.value)}
                                    name=''
                                    className='border rounded px-4 p-2 h-32'
                                    placeholder='What is this voice suitable for?'
                                    id=''
                                    required
                                />
                                <label htmlFor='' className='mb-2 mt-6'>
                                ...
                                <div className='flex flex-col gap-2'>
                                    <div className='flex flex-row flex-wrap gap-2'>
                                        {labels.map((item, index) => (
                                            <p
                                                className='bg-stone-200 p px-4 rounded-full'
                                                key={index}
                                            >
                                                {item}
                                            </p>
                                    ...
                                    <div className='flex flex-row gap-2'>
                                        <input
                                            className={`${
                                                invalidLabels ? 'border-red-700' : ''
                                            } px-4 p-2 w-full border rounded`}
                                        ...
                                        <button onClick={addLabel}>
                                            <PlusCircleIcon
                                                className={`${
                                                    invalidLabels ? 'text-red-700' : 'text-black'
                                ...
                                <label htmlFor='' className='mb-2 mt-6'>
                                    Royalty Fee for 10K Words
                                </label>
                                <div className='flex'>
                                    <input
                                        onChange={(e) => setRoyalty(e.target.value)}
                                        type='text'
                                        className='border rounded px-4 p-2 w-full'
                            ...
                            <div className='w-full'>
                                <div className='h-40 w-full rounded-lg border border-neutral-400 border-dashed relative mt-14'>
                                    <div className='flex flex-col pointer-events-none absolute items-center justify-center h-full w-full cursor-pointer'>
                                        <img
                                            ...
                                            src='/assets/images/music-plus.png'
                                            alt=''
                                        />
                                        <p className='mb-2'>
                                    ...
                                    <input
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                            handleChangeInputFile(e.target.files)
                                        }
                                        required
                                    />
                                </div>
                                ...
                                <div className='flex flex-col gap-2 mt-2'>
                                    {files.length < 0
                                        ...
                                        : files.map((item, index) => (
                                              <div
                                                  key={index}
                                                  className='flex border rounded-lg p-2 w-full justify-between'
                                              >
                                                  <p>{item[0].name}</p>
                                                  <button onClick={() => removeFile(item[0].name)}>
                                ...
                                <button
                                    type='button'
                                    onClick={() => handleChangeProceed()}
                                    className='rounded-lg p-2 mt-8 w-full bg-black text-white text-base'
                                >
                                    Generate VoiceCard NFT Collection
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col my-auto p-6 mx-auto items-center rounded-lg bg-white z-10 w-[600px]'>
                        ...
                        <div className='flex flex-col items-center p-12 gap-8'>
                            <h1 className='text-4xl text-center'>
                                Are you confirm to generate your VoiceCard?
                            </h1>
                            <img className='h-64 w-64' src='/assets/images/nft-card.png' alt='' />
                            <button
                                onClick={async () => {
                                    paramGeral.set(false);
                                    paramGeral.setOpenAiCloningProgress(true);
                                    await sendNewVoicecardNFT();
                                    paramGeral.setIsGenerated(true);
                                }}
                                className='text-xl p-4 w-96 rounded-lg text-white bg-black'
                            >
                                Proceed to generate NFT collection
                            </button>
                        </div>
                    </div>
                </div>
