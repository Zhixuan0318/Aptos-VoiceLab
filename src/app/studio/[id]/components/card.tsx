    ...
    return (
        <div className="flex flex-col border rounded-lg p-4 justify-between">
            <div className="flex flex-col">
                ...
                <div className="grid grid-cols-3 mb-8 text-sm gap-2">
                    {paramGeral.item.characteristics.map((item: string, index: number) =>
                        <p key={index} className="bg-neutral-200 text-xs lg:text-sm  p-1 w-full rounded-full text-center">{item}</p>
                    )}
                </div>
            ...
            <div className="flex flex-row gap-2 text-xs">
                <button className="flex flex-row gap-2 items-center justify-center w-full px-4 p-2 rounded-lg border border-black">
