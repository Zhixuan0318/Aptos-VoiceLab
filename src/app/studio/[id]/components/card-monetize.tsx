    ...
    return (
        <div className='flex flex-col border rounded-lg p-4 justify-between'>
            <div className='flex flex-col justify-between h-full'>
                <div className='flex flex-col'>
                    ...
                    <p className='text-xs  mt-2'>
                        By{' '}
                        {paramGeral.item.by.slice(0, 5) + '...' + paramGeral.item.by.slice(59, 64)}
                    </p>
                    <p className='text-neutral-500 text-sm mt-4 mb-4'>
                        {paramGeral.item.description}
                    </p>
            ...
            <div className='flex flex-row gap-2 text-xs'>
                <Link
                    href={`/studio/${idPath}/marketplace#${paramGeral.item.voice_id}`}
                    className='w-full text-center px-4 p-2 rounded-lg bg-black text-white'
                >
                    View On Marketplace
                </Link>
            </div>
