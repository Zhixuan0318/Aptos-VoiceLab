...
export default function MintVoiceCard(paramGeral: any) {
    const { mintCard } = useAptos();

    async function setMintToUse(tokenId: string) {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/changemint`, {
                ...
                body: JSON.stringify({
                    ...
                    voice_id: paramGeral.mintVoiceCard.voice_id,
                    tokenId: paramGeral.info[1],
                    name: paramGeral.item.name,
                }),
            });
            await response.json();
        } catch (error) {}
    ...
    return (
        ...
        >
            ...
            <div className='flex flex-col my-auto p-6 mx-auto items-center rounded-lg bg-white z-10'>
                <button onClick={() => paramGeral.set(false)} className='flex w-full justify-end'>
                ...
                <div className='flex flex-col items-center p-12'>
                    ...
                    <div className='flex flex-col shadow-around-purple rounded-lg p-4 mb-8'>
                        ...
                        <div className='flex flex-row items-center gap-2 text-sm mt-2'>
                            ...
                            <p className='items-center'>{paramGeral.info[0]} APT / 10,000 Words</p>
                        </div>
                    </div>
                    <button
                        onClick={async () => {
                            paramGeral.set(false);
                            paramGeral.setMintinInProgress(true);
                            await setMintToUse(paramGeral.info[1]);
                            paramGeral.setIsMinted(true);
                        }}
