...
export default function GeneratingNft(paramGeral: any) {
    ...
    useEffect(() => {
        if (paramGeral.isGenerated) {
            setTimeout(() => {
                setCarregando(false);
            }, 1000);
        }
    }, [paramGeral.isGenerated]);
        ...
        >
            ...
            <div className='flex flex-col my-auto p-6 mx-auto items-center rounded-lg bg-white z-10'>
                <div className='flex flex-col items-center p-12'>
                    <h1 className={`${carregando ? 'hidden' : 'flex'} text-4xl text-center`}>
                        VoiceCard NFT Collection
                        <br /> Generated Successfully!
                    </h1>
                    <div className={`${carregando ? 'flex' : 'hidden'} flex-col`}>
                        ...
                        <p className='max-w-[640px] text-center'>
                            ...
                            affect the process. Be patient and your VoiceCard NFT collection will be
                    ...
                    <Link
                        href={`/studio/${paramGeral.idPath}/marketplace/#${paramGeral.voice_id}`}
                        onClick={() => {
                            paramGeral.set(false);
                        }}
                        className={`${
                            carregando ? 'opacity-20 pointer-events-none' : 'opacity-100'
                        } text-center text-xl p-4 w-96 rounded-lg text-white bg-black`}
                    >
